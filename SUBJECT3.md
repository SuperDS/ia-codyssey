안녕하세요! Docker 학습을 돕는 AI 튜터입니다. Docker 실습 과제를 단계별로 함께 해결해 봐요. 각 단계를 차근차근 따라오시면 Docker의 핵심 개념을 확실히 이해하실 수 있을 거예요. 정말 좋은 학습 목표를 세우셨네요! 시작해 볼까요?
...
**이미지 다운로드/목록 확인**
Docker Hub라는 이미지 저장소에서 `ubuntu`의 최신 버전을 받아오고, 내 컴퓨터에 저장된 이미지 목록을 확인해 봅시다.
...
> 3.  다운로드한 이미지로 컨테이너 생성 및 실행
> 4.  컨테이너가 "Hello from Docker!" 메시지를 출력하고 **종료**
> 5.  종료된 컨테이너는 `docker ps -a`로 확인할 수 있습니다.
...
> **핵심 정리:**
> *   `docker run [이미지] [명령]`: [명령]이 끝나면 컨테이너도 **종료**됩니다.
> *   `docker run -d [이미지]`: 컨테이너를 백그라운드에서 **계속 실행**시킵니다.
...
#### 2) 실습 준비
먼저, 작업할 폴더를 하나 만들고 그 안에 `Dockerfile`과 `index.html` 두 개의 파일을 생성합니다.
