...
---
### 프로젝트 디렉토리
* result 폴더 : 작업 진행하면서 필요한 스크린샷 결과 저장
* src 폴더 : 도커 index.html 테스트용 소스 파일 저장
* 도커파일과  readme파일은 쉽게 볼 수 있게 제일 바깥 폴더에 저장함

---
### 컨테이너 내부 포트 직접 접속 이슈
컨테이너 내부 포트에 직접 접속할 수 없는 이유는, 컨테이너가 호스트와 분리된 네트워크 공간에서 동작하기 때문이다.
따라서 외부에서 접속하려면 -p 호스트포트:컨테이너포트 형태의 포트 매핑이 필요하다. 예를 들어 -p 8080:80은 호스트의 8080 포트를 컨테이너의 80 포트와 연결해 브라우저 접속이 가능하게 한다

---
### 절대경로 / 상대경로
절대 경로는 /부터 시작하는 전체 경로로, 현재 위치와 상관없이 항상 같은 대상을 가리킵니다.
상대 경로는 현재 작업 디렉토리를 기준으로 해석되는 경로입니다.

ex
현재 위치가 어디든 상관없이
cd /home/user/codyssey/practice
바로 이동
